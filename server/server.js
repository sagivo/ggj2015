...
app.post('/init', function(req, res){  
  ...
  if (line.length > 0) { //join
    ...
    players[player1] = players[player2] = {game_id: game_id};
  } else { //wait
    line.push(player1);
    players[player1] = {game_id: null};
  }
  res.send(s); // player1 | player2 | gameid
});
...
app.post('/actions/:player_id', function(req, res){
  players[req.player_id].actions = req.body.actions;
  res.send('ok');
});
...
app.post('/lvlup/:game_id', function(req, res){
  ...
  if (players[game.player1].actions){
    players[game.player1].actions = players[game.player2].actions = null;
    game.lvl++;
  }
  res.send('ok');
});
